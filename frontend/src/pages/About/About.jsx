import React from "react";
import { Feather, BookOpen, Newspaper, Star } from "lucide-react";

const AboutSection = () => {
    return (
        <div className="min-h-screen bg-[#F2E8CF] font-[Cormorant] py-16 px-4">
            {/* Main Container */}
            <div className="max-w-4xl mx-auto">
                {/* Header Section */}
                <div className="text-center mb-16">
                    <Feather className="w-16 h-16 mx-auto text-[#8B4513] mb-4" />
                    <h1 className="text-4xl md:text-5xl font-[Cinzel] text-[#8B4513] tracking-wider mb-4">
                        About LumiFeed
                    </h1>
                    <div className="w-24 h-1 bg-[#8B4513]/20 mx-auto" />
                </div>

                {/* Introduction Card */}
                <div className="bg-white/40 backdrop-blur-md rounded-lg p-8 md:p-12 shadow-lg border border-[#8B4513]/20 mb-12">
                    <h2 className="text-3xl font-[Cinzel] text-[#8B4513] mb-6 text-center">
                        Your Modern News Reader
                    </h2>
                    <p className="text-lg text-[#8B4513]/80 leading-relaxed mb-6">
                        Welcome to LumiFeed, your modern and clutter-free news
                        reading platform. This isn't your traditional news
                        source; instead, it's best described as a "News Reader."
                    </p>
                    <div className="border-l-4 border-[#8B4513]/20 pl-6 my-8 py-2">
                        <p className="text-xl italic text-[#8B4513]/90">
                            "Where ancient wisdom meets modern convenience"
                        </p>
                    </div>
                </div>

                {/* Features Grid */}
                <div className="grid md:grid-cols-2 gap-8 mb-12">
                    <div className="bg-white/40 backdrop-blur-md rounded-lg p-6 shadow-lg border border-[#8B4513]/20">
                        <div className="flex items-center mb-4">
                            <Newspaper className="w-8 h-8 text-[#8B4513] mr-3" />
                            <h3 className="text-xl font-[Cinzel] text-[#8B4513]">
                                Curated Sources
                            </h3>
                        </div>
                        <p className="text-[#8B4513]/80">
                            Choose from premium news sources like BBC, CNN,
                            Indian Express, and more. Select your preferred
                            genre and get instant access to clean, ad-free
                            content.
                        </p>
                    </div>

                    <div className="bg-white/40 backdrop-blur-md rounded-lg p-6 shadow-lg border border-[#8B4513]/20">
                        <div className="flex items-center mb-4">
                            <Star className="w-8 h-8 text-[#8B4513] mr-3" />
                            <h3 className="text-xl font-[Cinzel] text-[#8B4513]">
                                Smart Summaries
                            </h3>
                        </div>
                        <p className="text-[#8B4513]/80">
                            Get high-quality summaries for every article,
                            helping you quickly grasp key points without reading
                            through lengthy content.
                        </p>
                    </div>
                </div>

                {/* How It Works Section */}
                <div className="bg-white/40 backdrop-blur-md rounded-lg p-8 md:p-12 shadow-lg border border-[#8B4513]/20 mb-12">
                    <h2 className="text-3xl font-[Cinzel] text-[#8B4513] mb-6 text-center">
                        How It Works
                    </h2>
                    <div className="space-y-6">
                        <div className="flex items-start">
                            <div className="w-8 h-8 rounded-full bg-[#8B4513] text-white flex items-center justify-center flex-shrink-0 mt-1">
                                1
                            </div>
                            <p className="ml-4 text-lg text-[#8B4513]/80">
                                Choose your favorite news agency from our
                                curated list of trusted sources.
                            </p>
                        </div>
                        <div className="flex items-start">
                            <div className="w-8 h-8 rounded-full bg-[#8B4513] text-white flex items-center justify-center flex-shrink-0 mt-1">
                                2
                            </div>
                            <p className="ml-4 text-lg text-[#8B4513]/80">
                                Select your preferred genre - from politics to
                                sports and everything in between.
                            </p>
                        </div>
                        <div className="flex items-start">
                            <div className="w-8 h-8 rounded-full bg-[#8B4513] text-white flex items-center justify-center flex-shrink-0 mt-1">
                                3
                            </div>
                            <p className="ml-4 text-lg text-[#8B4513]/80">
                                Get instant access to clean, ad-free articles
                                with smart summaries.
                            </p>
                        </div>
                    </div>
                </div>

                {/* Closing Section */}
                <div className="text-center">
                    <BookOpen className="w-12 h-12 mx-auto text-[#8B4513] mb-4" />
                    <p className="text-lg text-[#8B4513]/80 italic mb-6">
                        Thank you for visiting LumiFeed. Enjoy your
                        distraction-free news reading experience!
                    </p>
                    <div className="w-24 h-1 bg-[#8B4513]/20 mx-auto" />
                </div>
            </div>

            <style jsx global>{`
                @import url("https://fonts.googleapis.com/css2?family=Cinzel:wght@400;500;600&family=Cormorant:ital,wght@0,400;0,500;0,600;1,400;1,500&display=swap");
            `}</style>
        </div>
    );
};

export default AboutSection;
