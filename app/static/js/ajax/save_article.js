...
function saveArticleInReadLater() {
    const ReadLaterCollectionBtns = document.querySelectorAll(
        ".read_later_collection",
    );
...
function saveArticleInOtherCollections() {
    const ReadLaterCollectionBtns =
        document.querySelectorAll(".other_collections");
    const collectionsNameInputs = document.querySelectorAll(
    ...
    ReadLaterCollectionBtns.forEach((btn, index) => {
        btn.addEventListener("click", () => {
            ...
            if (collectionsNameInputs.length === 0) {
                displayMessage(
                    "Please log in first to save articles. Not logged in.",
                    "error",
                );
                return;
            }
            ...
            if (
                ...
                !collectionsInput.style.display
            ) {
