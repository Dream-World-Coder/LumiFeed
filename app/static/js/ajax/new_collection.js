function addCollectionToDOM(newCollection) {
    ...
    const doc = parser.parseFromString(newCollection, "html");
...
function addNewCollection() {
    ...
    newCollectionBtn.addEventListener("click", () => {
        ...
        if (userInput !== null && userInput.trim() !== "") {
            ...
            displayMessage(
                "Collection name is too long. Max length is 99 characters.",
                "error",
            );
        } else {
            displayMessage(
                "Collection creation cancelled or no name entered.",
                "message",
            );
        }
    });
